...
###    September 2016
###    Updated Oct 2017  - DK
###    Updated Aug/Sep 2018 - DK
###    2018 modelling by L. Nasmith
###    Update June 2020 JS 
###........................................###
...
#NOTE: when assessment run in Aug 2020, tested assumption of what would happen if set the catch from the 2018/2019 fishing season catch to be 339 to see impact of the approx 130 mt that came from digby next outside the modelled area 
#raw.dat$C[raw.dat$YearSurvey == 2018] <- 339  #Results of this model run saved in folder: Model_test_subsetcatchinModelledAreain2019
...
SPA1A.dat <-  as.list(raw.dat[,c("C","N","I","I.cv","IR","IR.cv","ratiolined","g","gR","clappers")])
SPA1A.dat$NY <- NY
...
save(list = paste0("Spa1A.", max(yrs)), file=paste0(direct,"/",assessmentyear, "/Assessment/Data/Model/SPA",area,"/SPA1A_Model_",max(yrs),".RData"))
#save(paste0("Spa1A.", max(yrs)), file = paste0(direct,"/",assessmentyear, "/Assessment/Data/Model/SPA",area,"/SPA1A_Model_",max(yrs),".RData"))
...
temp <- print(mod.res)
write.csv(temp, paste0(direct,"/",assessmentyear,"/Assessment/Data/Model/SPA",area,"/Spa1AModelOutput.csv"))
...
summ.Spa1A <- summary(mod.res)
dump('summ.Spa1A', paste0(direct,"/",assessmentyear,"/Assessment/Data/Model/SPA",area,"/Spa1AModelsummaryObj.R"))
...
#plot(Spa1A.2017, type="Survey.est")
SSModel.plot.median(mod.res, type="Survey.est", french=TRUE)
dev.off()
...
RP.labels <- data.frame(region = c("Healthy","Cautious","Critical"),x.pos = c(2013,2015,2015),size = c(1,1,1))
SSModel.plot.median(mod.res, ref.pts=c(LRP,USR), Catch.next.year=catch.next.year, pred.lim=0.2,
                    g=mod.res$data$g[mod.res$data$NY],gR=mod.res$data$gR[mod.res$data$NY],log.R=F,RP.labels=RP.labels)
dev.off()
...
RP.labels <- data.frame(region = c("Healthy","Cautious","Critical"),x.pos = c(2013,2015,2015),size = c(1,1,1))
SSModel.plot.median(mod.res, ref.pts=c(LRP,USR), Catch.next.year=catch.next.year, pred.lim=0.2,
                    g=mod.res$data$g[mod.res$data$NY],gR=mod.res$data$gR[mod.res$data$NY],log.R=F,RP.labels=RP.labels, french=TRUE)
dev.off()



#plot(Spa1A.2017, ref.pts=c(530,750), Catch.next.year=125, pred.lim=0.1)
#windows()
...
SSModel.plot.median(mod.res, ref.pts=c(LRP,USR), Catch.next.year=catch.next.year, pred.lim=0.2,
                    g=mod.res$data$g[mod.res$data$NY],gR=mod.res$data$gR[mod.res$data$NY],log.R=NULL,RP.labels=RP.labels,cex=1.4, french=FALSE)
dev.off()
...
#jpeg(paste0(direct,"/",assessmentyear,"/Assessment/Figures/Model/SPA",area,"/Biomass_figure_for_Document_",area,"_FR.jpg"),width=11,height=8,units = "in",res=920)
# default growth parameters
#RP.labels.FR <- data.frame(region = c("Healthy","Cautious","Critical"),x.pos = c(2017,2017,2017),size = c(1.4,1.4,1.4))
#SSModel.plot.median(mod.res, ref.pts=c(LRP,USR), Catch.next.year=catch.next.year, pred.lim=0.2,
     #               g=mod.res$data$g[mod.res$data$NY],gR=mod.res$data$gR[mod.res$data$NY],log.R=NULL,RP.labels=RP.labels.FR,cex=1.4)
#dev.off()
...
#plot(Spa1A.2017, type="Prior.Post")
...
{
  if(pred.yr[j] < 2016)   load(file = paste0(direct,"/Model_results_2009_2015/SPA1A/SPA1A_",pred.yr[j],".RData"))  
  if(pred.yr[j] %in% c(2016, 2017, 2018))  load(file = paste0(direct, "/",pred.yr[j],"/",pred.yr[j]," Assessment/SPA1A/ModelOutput/SPA1A_Model_",pred.yr[j],".RData")) 
  #if(pred.yr[j] %in% c(2019, 2020, 2021)) load(file = paste0(direct,"/",assessmentyear,"/Assessment/Data/Model/SPA",area,"/SPA1A_Model_",pred.yr[j],".RData"))
  if(pred.yr[j] == 2019)   load(file = paste0(direct,"/2020/Assessment/Data/Model/SPA",area,"/SPA1A_Model_",pred.yr[j],".RData")) 
  ...
  if(pred.yr[j] == 2020)   load(file = paste0(direct,"/2021/Assessment/Data/Model/SPA",area,"/SPA1A_Model_",pred.yr[j],".RData"))
  if(pred.yr[j] >= 2021)   load(file = paste0(direct,"/",pred.yr[j],"/Assessment/Data/Model/SPA",area,"/SPA1A_Model_",pred.yr[j],".RData")) 
}
...
#! NOTE: warning message from plot is due to differences in the length of the chains between years, do not worry about it!
# "In cbind(temp, x[[i]]$B.cur, x[[i]]$B.next):  number of rows of result is not a multiple of vector length (arg 2)"
...
tt <- Sys.time()
save(LRP, USR, catch.next.year, min.neff, Rhat, temp, stats.output, summ.Spa1A, decision, decision.table,  prob.above.USR, prob.above.LRP,
...
png(paste0(direct,"/",assessmentyear,"/Assessment/Figures/Model/SPA1A/SPA1A_population_numbers_panel_indices",surveyyear,".png"), type="cairo", width=20, height=15, units = "cm", res=300)
I.N.plot.2
dev.off() 
...
png(paste0(direct,"/",assessmentyear,"/Assessment/Figures/Model/SPA1A/SPA1A_population_number_index",surveyyear,".png"), type="cairo", width=20, height=15, units = "cm", res=300)
I.N.plot
dev.off() 
