...
###
###    L.Nasmith, July 2016
###    Modified, J.Sameoto Sept, 2017
###    Updated Oct 2017  - DK
###   Updated May and Aug 2018 - DK
...
area <- 3  #this would be the SPA, for entries options are to use: 1A, 1B, 3, 4, or 6  

#reference points 
LRP <- 600
USR <- 1000

# Put in the catch for next year in the 2023 slot of the "C" data
...
CreateExcelModelFile(direct = direct, 
                     ...
                     area = 3, LastYearsModelRData = "SPA3_Model_2024", 
                     savefile = T)
...
niter = 400000 # default = 100000
nchains = 3    # default =33
nburnin = 200000  # default = 50000
nthin = 10       # default = 10
...
raw.dat <- read.xlsx(paste0(direct,"/",assessmentyear,"/Assessment/Data/Model/SPA",area,"/SPA3_ModelData_R_2025-10-20.xlsx"),sheet = "AlignedForModel",cols=1:13)
str(raw.dat)
raw.dat$C <- as.numeric(raw.dat$C)
raw.dat

#Test of model run with no data for 2020 -- since no survey in 2020 -- insert NAs for 2020 for survey related data: 
#raw.dat[raw.dat$YearSurvey==2020, c( "I", "IR")] <- NA
#raw.dat[raw.dat$YearSurvey==2020, c( "I", "IR",  "I.cv",  "IR.cv")] <- NA
#raw.dat[raw.dat$YearSurvey==2020, c( "I", "IR",  "I.cv",  "IR.cv", "clappers")] <- NA

# Put in the catch for next year slot of the "C" data
...
raw.dat
# Get the years you want to run, data starts in 1996 so can't go older than that.
...
SPA3.dat <- as.list(raw.dat[,c("C","N","I","I.cv","IR","IR.cv","ratiolined","g","gR","clappers")])
SPA3.dat$NY <- NY

# This makes a function for the SPA3 inits, built this for 3 chains, if you change the number of chains you will need to change the number of lists (each list is inits 
# for 1 chain)
SPA3.inits <- function(NY)
{
...
# Save this when you are happy with the results; #rename so can use prediction evaluation 
save(list = paste0("Spa3.new.", max(yrs)), file=paste0(direct,"/",assessmentyear, "/Assessment/Data/Model/SPA",area,"/SPA3_Model_",max(yrs),".RData"))
#Spa3.new.2021 <- Spa3.model  #note the YYYY part needs to be updated each year 

# Save this when you are happy with the results
#save(Spa3.model,file = paste0(direct,"/",assessmentyear,"/Assessment/Data/Model/SPA",area,"/SPA3_Model_Oct62021_minggR",max(yrs),".RData"))
...
temp<-print(mod.res)
write.csv(temp, paste0(direct,"/",assessmentyear,"/Assessment/Data/Model/SPA",area,"/spa3ModelOutput.csv"))
...
summ.Spa3 <- summary(mod.res)
dump('summ.Spa3', paste0(direct,"/",assessmentyear,"/Assessment/Data/Model/SPA",area,"/spa3ModelsummaryObj.R"))
...
#plot(Spa3.new.2017, type="Survey.est")
...
#plot(Spa3.new.2017, type="Prior.Post")
...
#pred.yr <- c(2009:2019,2021:max(yrs)) #update in 2021 to deal with missing 2020 data 
pred.yr <- c(2009:max(yrs)) 
num.pred.years <- length(pred.yr)
t.yrs <- yrs
for(i in 1:num.pred.years)
{
  if(pred.yr[i] < 2016)   load(file = paste0(direct,"/Model_results_2009_2015/SPA3/SPA3_",pred.yr[i],".RData"))  
  if(pred.yr[i]%in% c(2016, 2017, 2018))  load(file = paste0(direct,"/",pred.yr[i],"/",pred.yr[i]," Assessment/SPA3/ModelOutput/SPA3_Model_",pred.yr[i],".RData"))  
  if(pred.yr[i] == 2019)   load(file = paste0(direct,"/2020/Assessment/Data/Model/SPA",area,"/SPA3_Model_",pred.yr[i],".RData")) 
  #Note no model run for 2020 since no survey in 2020 but ran the model with imputed data since needed for prediction evals 
  if(pred.yr[i] == 2020)   load(file = paste0(direct,"/2021/Assessment/Data/Model/SPA",area,"/SPA3_Model_",pred.yr[i],".RData")) 
  if(pred.yr[i] >= 2021)   load(file = paste0(direct,"/",pred.yr[i],"/Assessment/Data/Model/SPA",area,"/SPA3_Model_",pred.yr[i],".RData")) 
}
...
#Model output from 2020 not from actauly survey so do not show 2020 "actual" (blue) or 2021 predicted (white) which would be from 2020 predicted 1 year ahead on figures: 
#note not entirely sure this is the right object/year to NA but cann't test until fix the eval.predict() function in SSModel to remove NAs from calculation in quantile. 
...
#Need to edit eval.predict function to have na.rm = TRUE in quantile call, or even in apply call that calls the quantile function - this is done using modified function in eval_pred_inshore_replacement.R (sourced) 
#Combined.runs.predicted$`2021`$B.cur <- rep(NaN,length(Combined.runs.predicted$`2021`$B.cur))  #This is 2020 "current survey" data 
#Combined.runs.predicted$`2021`$B.next <- rep(NaN,length(Combined.runs.predicted$`2021`$B.next)) #This is 2020 "predicted survey" data predicted to 2021 


# Plot this PREDICTED prediction-evaluation plot.
#windows()
png(paste0(direct,"/",assessmentyear,"/Assessment/Figures/Model/SPA",area,"/Prediction_evaluation_figure_predicted_",area,".png",sep=""),width=8,height=11,units = "in",res=920)
...
#error from trying this with eval.predict is  < Error in quantile.default(newX[, i], ...) : missing values and NaN's not allowed if 'na.rm' is FALSE > 
#Need to edit eval.predict function to have na.rm = TRUE in quantile call, or even in apply call that calls the quantile function - this is done using modified function in eval_pred_inshore_replacement.R (sourced) 
#Combined.runs.actual$`2021`$B.cur <- rep(NaN,length(Combined.runs.predicted$`2021`$B.cur))  #This is 2020 "current survey" data 
#Combined.runs.actual$`2021`$B.next <- rep(NaN,length(Combined.runs.predicted$`2021`$B.next)) #This is 2020 "predicted survey" data predicted to 2021 

png(paste0(direct,"/",assessmentyear,"/Assessment/Figures/Model/SPA",area,"/Prediction_evaluation_figure_actual_",area,".png"),width=8,height=11,units = "in",res=920)
...
tt <- Sys.time()
save(LRP, USR, catch.next.year, min.neff, Rhat, temp, stats.output, summ.Spa3, decision, decision.table,  prob.above.USR, prob.above.LRP,
